...
int RANGEFILTERSIZE = 8000;  // size of synopsis in nodes
...
int main(int argc, char* argv[]) {
	...
	int dim;
	  ...
	  /* start outputting stuffs */
	         ...
	         fprintf(results,"DIMS \t\tQD \t\tKD \t\tKDA \t\tCC \t\tZC \t\tHC \t\tBL Correlated data \n");
  ...
  cout<<"Experiment 3 [ varying dimensions ] - correlated/anti-correlated experiments"<<endl;
  ...
  int ar = qGen.getAverageArea();
  cout<<"generated queries with area:"<<ar<<endl;
  cout<<"DOMAIN:"<<DBDOMAIN<<endl;
  cout<<"db domain:"<<db.curve->getDomain()<<endl;
    ...
    Query qTrain2(domains,2*TRAININGQUERIES/3,
	 ...
	 if(useH)
		...
		printf("SIZE \t\tQD \t\tKD \t\tKDA \t\tCC \t\tZC \t\tHC \t\tBL \n");
		 ...
		 for (int i=0; i<EXPQUERIES; i++)
		   {
				...
				Query::QueryMD_t q = qg.nextQueryMD();
		 ...
		 cout<<"Average area: "<<qg.getAverageArea()<<endl;
		 double avf = qg.getAverageArea();
		  ...
		  multi.stats.print();
		  cout<<endl<<"KDA:"<<endl;
		  multi_alt.stats.print();
		  cout<<endl<<"c curve:"<<endl;
		  sync.stats.print();
		  cout<<endl<<"z curve:"<<endl;
		  synz.stats.print();
		  cout<<endl<<"h curve:"<<endl;
		  synh.stats.print();
		  //getchar();
