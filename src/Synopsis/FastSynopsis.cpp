#include "FastSynopsis.h"

//typedef unsigned long long uint64;


void FastSynopsis::set_used(node * n, int used_value) {

  n->used_counter = used_value;

}

void FastSynopsis::modify_used(node *n, int to_add) {
...
void FastSynopsis::sanity_check(node * n) {
...
void FastSynopsis::sanity_check() {
	sanity_check(root);
}

int FastSynopsis::get_used(const node& n) {
...
void FastSynopsis::learn_from_tn(uint64 left, uint64 right) {
...
void FastSynopsis::increment_used(node * n, uint64 left, uint64 right, int to_add) {
...
void FastSynopsis::perfect(Database * database)
{
	Query::Query_t q;
	q.left = 0;
	...
	{
		/*if(i%100 == 0)
			cout<<"perfect learning from "<<i<<" of "<<empty.size()<<endl;*/
		learn_from_fp(empty[i].left,empty[i].right);
	}

}

bool FastSynopsis::navigate(uint64 left, uint64 right) {
	return navigate(root,left,right);
}
void FastSynopsis::learn_from_fp(uint64 left, uint64 right) {
...
uint64 FastSynopsis::getDomain() {
	return root->right;
}

void  FastSynopsis::print_node(node * n) {
...
void FastSynopsis::mark_range(node * n, uint64 left, uint64 right, bool value) {
...
void FastSynopsis::escalate(uint64 bound, bool is_left, bool target_value) {
...
void FastSynopsis::assert_is_leaf(node * n) {
...
uint64 FastSynopsis::get_middle(uint64 l, uint64 r) {

  return (r+l)>>1;

}

FastSynopsis::~FastSynopsis() {

	free_nodes(root);
}

void FastSynopsis::free_nodes(node * n) {
...
void FastSynopsis::split(node * n) {
...
int FastSynopsis::space_of_node(node * n)
{
...
void FastSynopsis::initialize_node(node * n,uint64 left, uint64 right,bool is_leaf,
...
void FastSynopsis::end_training_phase() {
 this->is_training_phase = false;
 this->stats.reset();
}

void FastSynopsis::reset_training_phase() {
 this->is_training_phase = true;
 this->stats.reset();
}

void FastSynopsis::print_size(node * n, int& nodes, int& leaves) {
	if(n->is_leaf) {
		leaves++;
		//print_node(n);
		return;
...
FastSynopsis::FastSynopsis(int space_of_counters, uint64 domain, Database * db) {
 ...
 bool FastSynopsis::contains(node * n, uint64 key) {

   return (n->left<=key && n->right>=key);
 }

 void FastSynopsis::assert_children_integrity(node * n) {
 ...
 FastSynopsis::node * FastSynopsis::getLeaf(uint64 key) {
 ...
 bool FastSynopsis::navigate_internal(node * n, uint64 key, node ** leaf_node) { //point query
 ...
 bool FastSynopsis::navigate(node *n, uint64 left, uint64 right) {
 ...
 bool FastSynopsis::handle_query(uint64 left, uint64 right,bool actual_result,  bool do_adapt) {
 ...
 void FastSynopsis::learn_from_tp(uint64 left, uint64 right) {
	 //TODO write me -- optional

 }

 void FastSynopsis::evict(node * n) {
...
last_evicted_left = n->left;
 ...
 bool FastSynopsis::is_logical_node(const node & n) {
	 return (!n.is_leaf && n.left_child->is_leaf && n.right_child->is_leaf);
 }

 bool FastSynopsis::found_last_evicted(node * n) {
 ...
 void FastSynopsis::truncate(node * n, int target, bool * skipped_last) {
...
void FastSynopsis::recordNewKeys(uint64 * keys,int num_keys,int strategy) {
...
/*
 ...
 void FastSynopsis::truncate(int target) {
...
if(verbose)
 ...
 void FastSynopsis::merge(node * left, node * right) { //free?
