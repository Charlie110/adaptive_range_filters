...
// *************************************************************************** 
	...
	{ 
	if (argc != 5) { 
	printf("generate - generate test data for multi dimensional trie :)) \n\n"); 
	printf("Syntax: generate <dimensions> <correlation> <nr points> <data filename>\n"); 
	printf(" wif correlation = E(qually) | C(orrelated) | A(nti-correlated)\n\n"); 
	} else { 
